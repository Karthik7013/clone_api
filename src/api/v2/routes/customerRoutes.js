...
const { customerAuthorized } = require('../../middleware/authorizationSample');
const customerRoutes = Router();




customerRoutes.get('/profile/:id', isAuthenticated(['customer']), customerAuthorized,
    async (req, res, next) => {
        try {
...
customerRoutes.put('/profile/:id', isAuthenticated(['customer']), customerAuthorized, async (req, res, next) => {
    try {
        const response = await customerProfileHandler(req);
        return res.status(200).json(successHandler(
            null,
...
customerRoutes.delete('/profile/:id', isAuthenticated(['customer']), customerAuthorized, async (req, res, next) => {
    try {
        const response = await customerProfileHandler(req);
        return res.status(200).json(successHandler(
            null,
...
customerRoutes.get('/application/:id', isAuthenticated(['customer']), (req, res, next) => {
